...
IdentifierName可以是Identifier、NullLiteral、BooleanLiteral或者keyword，在ObjectLiteral中，IdentifierName还可以被直接当做属性名称是用哪个。

仅当不是保留字的时候，IdentifierName会被解析为Identifier。

注意`<ZWNJ`和`<ZWJ>`是ES5新加入的两个格式控制字符，它们都是 0 宽的。
...
如果script 标签如果不加`type=“module”`，默认认为我们加载的文件是脚本而非模块，如果我们在脚本中写了 export，就会抛错。
